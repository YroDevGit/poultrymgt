<?php
    ...
    if(isset($_POST["productionsave"])){
        $f1 = $_POST['NumberOfEggs'];
        $f2 = $_POST['NumberOfCrack'];
        $f0 = $_POST['Date'];
        $tm = $_POST["time"];
        $a1 = $_POST['mini'];
        $a2 = $_POST['pewee'];
        ...
        $bool = hasResults("select * from production where date = '$f0'");



        if($bool==1){
$rz = queryx("update production set NumberOfEggs = $f1 , Numbercrack = $f2, egg_mini = $a1, egg_pewee = a2, egg_small = a3, egg_medium = a4, egg_large = a5, egg_xl = a6, egg_jumbo = a7, egg_brook = a8, egg_stain = a9, time = '$tm' where Date = '$f0'");
            if($rz){
        ...
        $myArray = array(
            "Date" => $_POST["Date"],
            "NumberOfEggs" => $_POST["NumberOfEggs"],
            "Numbercrack" => $_POST["NumberOfCrack"],
            "egg_mini" => $a1,
            "egg_pewee" => $a2,
            "egg_small" => $a3,
            "egg_medium" => $a4,
    ...
    if(isset($_POST['vacbtn'])){
            ...
            $vname = mysqli_real_escape_string($databaseObject->connect(),$_POST['vname']);
            $vprice = $_POST['vprice'];
            $vqty = $_POST['vtotal'];
            $vdate = $_POST['vdate'];
            $sql = "insert into vaccines values(null,'$vname','$vprice','$vqty','$vdate')";
            $result = $databaseObject->connect()->query($sql);
            if($result){
                header("refresh:0;url=../vaccine.php");
            }
    ...
    while($row = mysqli_fetch_array($result)){
        $totalEggsProduced = $row[0];
    }

    $query = "SELECT SUM(t.quantity)*30 FROM transaction t";
    $result = $databaseObject->connect()->query($query);
    while($row = mysqli_fetch_array($result)){
        $totalEggsSold = $row[0];
    }
